...
class AIVisionAnalyzer(GoogleDriveImageAnalyzer):
    ...
    def analyze_image_with_openai(self, image_path: str, survey_context: Dict) -> Dict:
        ...
        """
        if not openai.api_key:
            return self.analyze_image_with_ai(image_path, survey_context)
        ...
        try:
            ...
            survey_info = self._format_survey_context(survey_context)
            
            prompt = f"""
            Analyze this image in the context of a user survey response. Here's the survey data:
            
            {survey_info}
            
            Please provide a comprehensive analysis including:
            1. Detailed description of what you see in the image
            2. Objects, people, or activities detected
            3. Text visible in the image (if any)
            4. Emotional tone or sentiment conveyed
            5. How the image relates to the survey responses
            6. Any insights about user preferences or behavior
            7. Technical quality assessment (lighting, composition, etc.)
